...
export const JoinFamilyBanner = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Sucesso!',
        description: 'Voce foi vinculado a familia.',
      });
      setShowInput(false);
      setCode('');
    } catch (error: any) {
      ...
      <Card className="bg-blue-500/5 border-blue-500/20">
        <CardContent className="flex items-center justify-between p-4 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-foreground">Vincular a familia</p>
              <p className="text-sm text-muted-foreground">
                {isParent
                  ? 'Peca o codigo para o responsavel que criou a familia.'
                  : 'Peca o codigo para seus pais'}
              </p>
            </div>
          </div>
    ...
    <Card className="bg-blue-500/5 border-blue-500/20">
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="font-medium text-foreground">Digite o codigo familiar</p>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setShowInput(false)}>
            <X className="w-4 h-4" />
          </Button>
        </div>
